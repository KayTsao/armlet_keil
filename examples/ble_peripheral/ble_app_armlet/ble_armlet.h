 ...
 *          through the ble_armlet_battery_level_update() function.
 ...
 *       ble_armlet_on_ble_evt() from the @ref softdevice_handler callback.
 *
 * @note Attention! 
...
typedef struct
{
    ble_armlet_evt_type_t evt_type;                                  /**< Type of event. */
} ble_armlet_evt_t;

// Forward declaration of the ble_armlet_t type. 
typedef struct ble_armlet_s ble_armlet_t;

/**@brief Battery Service event handler type. */
typedef void (*ble_armlet_evt_handler_t) (ble_armlet_t * p_bas, ble_armlet_evt_t * p_evt);

/**@brief Battery Service init structure. This contains all options and data needed for
 *        initialization of the service.*/
typedef struct
{
    ble_armlet_evt_handler_t      evt_handler;                    /**< Event handler to be called for handling events in the Battery Service. */
    ble_srv_cccd_security_mode_t  sensor_char_attr_md;     /**< Initial security level for battery characteristics attribute */
    ble_gap_conn_sec_mode_t       sensor_report_read_perm; /**< Initial security level for battery report read attribute */
} ble_armlet_init_t;
...
struct ble_armlet_s
{
    ...
    ble_gatts_char_handles_t      sensor_handles;          /**< Handles related to the Battery Level characteristic. */
 ...
 *       ble_armlet_battery_level_update() must be called upon reconnection if the
