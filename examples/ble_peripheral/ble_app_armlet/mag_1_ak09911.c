...
#ifdef  IQIYI_B45100557A
  #define AK09911_CHIP_ID2     			(0x48)
#endif
#ifdef  IQIYI_B45100557C
  #define AK09911_CHIP_ID2     			(0x0C) 
#endif
...
#define AK09911_I2C_ADDR     AK09911_I2C_ADDR1
...
static bool mag_event_init(uint8_t evt, sTwiOper * pNext)
{
	...
	{
		mag_state  = 0;
		NRF_LOG_PRINTF("AK0991x(1) error\r\n");
	}
	mag_busy = 0;
...
void mag_1_measure(void)
{
	...
	else if (get_time_ms() >= mag_tick && !mag_busy)
	{
		...
		case 1:
			...
			break;
		case 2:
			NRF_LOG_PRINTF("AK0991x(1) ID1:%02X\r\n", mag_buffer[0]);
			...
			break;
		case 3:
			NRF_LOG_PRINTF("AK0991x(1) ID2:%02X\r\n", mag_buffer[0]);
			...
			{
				mag_state++;
				NRF_LOG_PRINTF("AK0991x(1) Init Finished\r\n");
			}
			break;
		default:
		  	mag_tick  = get_time_ms() + 10;
...
void mag_1_getraw(int16_t* pbuf)
{
